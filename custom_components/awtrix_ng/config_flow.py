...
class AwtrixConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_setup_profiles(
        ...
        api = AwtrixNgApi(
            ...
            username=self.awtrix_config[CONF_USERNAME] or None,
            password=self.awtrix_config[CONF_PASSWORD] or None,
            port=80,
        )

        try:
            info = await api.async_get_device()
...
async def async_discovery(hass: HomeAssistant) -> list[dict[str, Any]]:
    ...
    return await hass.async_add_executor_job(_discover_devices)


def _discover_devices() -> list[dict[str, Any]]:
    """Broadcast a UDP discovery request and collect replies (blocking, runs in executor)."""
    devices = []

    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM,
                        socket.IPPROTO_UDP) as sock:  # UDP
